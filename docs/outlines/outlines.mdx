...
- `Type`: <PropertyIcon name="dropdown" />**Dropdown**, Options: `Regular`/`Render Feature (URP)`
  - Default: `Regular`
...
- `World` renders the Size based on World Units.
  - *Keep in mind that Outline Size will remain the same regardless of Avatar Scale if using `World`.*
...
:::tip
Animate this Float value if you're planning to create toggles for Outline visibility on this Material. Animating the [Section Header](/general/locking.mdx#section-header-checkboxes) will not work!
:::
...
Enables tinting the Highlight based on the specified [Outline Texture](#outline-texture) configuration.
...
Enables the ability to modify the Outline Color in a similar fashion to [Color Adjust](/color-and-normals/color-adjust.mdx). This is applied directly after the main [Outline Color](#color), and will not affect other sections that modify the base Outline Color.
...
Which [Vertex Color](/vertex-options/vertex-colors.mdx) channel to use to control the Outline Z Offset.
