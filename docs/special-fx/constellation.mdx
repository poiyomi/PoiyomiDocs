...
Uses a [Global Mask](/modifiers/global-masks.mdx) to limit where the constellation should only appear in.

## Write to Global Mask

- `Type`: <PropertyIcon name="dropdown" />**Dropdown**, Options: `Off`/`1R`/`1G`/`1B`/`1A`/`2R`/`2G`/`2B`/`2A`/`3R`/`3G`/`3B`/`3A`/`4R`/`4G`/`4B`/`4A`

If set, constellations will write onto the selected [Global Mask](/modifiers/global-masks.mdx) channel.
