...
image: /img/special-fx/Emission_Example.png
...
Select which [Global Mask](/modifiers/global-masks.mdx) to use as the Emission Mask instead.
...
- `Type`: <PropertyIcon name="dropdown" />**Dropdown**, Options: `Smooth`/`Pulse`

Choice of blinking behavior to use. By default, a smooth blinking transition is used. If set to `Pulse`, it will instead function in a PWM or Duty blink cycle.
...
- `Type`: <PropertyIcon name="floatrange" />**Float**, Range: `0.0 - 1.0`
  - Default: `0.5`
  - Conditional: Requires [Mode](#mode) set to `Basic`

Sets the interval in how and when each blink is timed. This is similar to PWM or Duty cycle modes in electronic lighting modules.
