...
<a>
<img src="/img/extended-features/LilFur-Haishima-Test.png" alt="Fluffy Haishima"/>
</a>
...
This chooses the rendering mode to use for Lil Fur. Settings will be applied to the module's [Rendering](#lil-fur-rendering) properties.
...
Mask Texture in grayscale that determines the length of the Fur in various places on the UV space. Areas in darker grayscale will reduce the length based on the initial [Length](#length) value set.
...
- `Type`: <PropertyIcon name="texture" />**Data** Texture (`sRGB = OFF`)
  - Default: `_PoiyomiShaders/Textures/Fur/lil_noise_fur_2.png`
