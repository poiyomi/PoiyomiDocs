...
*The Thumbnail changes from time to time and is fed images from this website. The context of the Thumbnail has nothing to do with ThryEditor whatsoever. It's just there for fun.*
...
### Show animated tagged dots on headers

- `Setting`: **Toggle**
  - Default: `On`

Shows a colored dot (`•`) on the Section Header that has detected child properties currently tagged as `A` or `RA`. Useful for debugging.
...
Allows a custom suffix to be used on properties that are marked Renamed Animated (`RA`), overriding the default naming as outlined [here](/general/locking.mdx#rename-animated).
...
Allows setting and displaying of Notes on Section Headers in the shader UI. Can be helpful for users who wish to remember what section they're using the feature for.
...
- `Setting`: **Dropdown**, Options: `None`/`Normal`/`Detailed`/`Stack Traced`
  - Default: `Normal`

Sets the `ThryLogger` level in the scripts across Thry Editor. For the most part, this should be left at `Normal`.

Tech-savvy users who wish to know what the scripts are doing may set this to `Detailed` or `Stack Traced`, although this will cause the Log files to get very large!
