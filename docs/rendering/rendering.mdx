...
:::info
Many of the settings in this section are automatically-configured depending on which [Rendering Preset](/general/render-preset.mdx) you select.
:::
...
- `Type`: <PropertyIcon name="dropdown" />**Dropdown**, Options: `Off`/`On`
  - Default: Determined by [Rendering Preset](/general/render-preset.mdx)
...
Enables or Disables the GPU's Depth Clip Mode, which determines how the GPU handles clipping fragments of the Shader that are past the Near and Far Planes.
...
A fixed value to move the polygon away from or toward the camera, while keeping it visually the same size. Viewed as a constant depth bias against Z-fighting.
...
Stops close geometry clipping the near plane. This can be used to make geometry not disappear when very close to the camera.
...
If enabled, inverts the normal vectors of the mesh if you're looking at a backface. This ensures it renders correctly and fixes lighting when the camera is looking towards the backface.
...
:::danger
**This setting is automatically set by the [Rendering Preset](/general/render-preset.mdx) and should not be touched!**
