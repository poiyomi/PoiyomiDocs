...
:::tip
This slider can be animated to hide and show the decal. Animating the [Section Header](/general/locking.mdx#section-header-checkboxes) will not work!
:::
...
- `Type`: <PropertyIcon name="toggle" />**Toggle**
  - Conditional: Requires [Video Effects](/special-fx/video-effects.mdx) to be turned on.

If enabled, will apply [Video Effects](/special-fx/video-effects.mdx) to the Decal from it's global function.

:::info[Shader Limitation]
Video Effects [Brightness](/special-fx/video-effects.mdx#brightness) setting will override this module's Emissions, as they are not cross-compatible!
:::
...
Enables the Color Adjustment sliders in Decals.
...
Adjusts the saturation of the Decal. 

A value of `-1` will make the Decal fully de-saturate (grayscale). A value of `0` will not alter the color at all. A value greater than 0 will increase the saturation of the base color.

This is implemented as a lerp between the Decal and a grayscale version of the Decal.
...
Adjusts the intensity of the Decal's color reproduction and quality regardless of it's luminance.
...
Adjusts the brightness of the Decal. 
...
Adjusts the gamma of the Decal.

This can lerp the brightness of the Decal's color in regards to it's hue or tone. Lower values are more lighter, while higher values are more bolder.
...
1. Ensure you have the `AudioLinkAvatar` Prefab in your Scene, as described [here](/audio-link/audio-link.mdx#how-to-test-audio-link-using-poiyomi-shaders).
...
Select which [Global Mask](/modifiers/global-masks.mdx) to send your Decal's effects onto the specified Global Mask.

### Show On Outlines

- `Type`: <PropertyIcon name="toggle" />**Toggle**

If enabled, will show the Decal on [Outlines](/outlines/outlines.mdx).
