...
To use this section, [Cull](/rendering/rendering.mdx#cull) must be set to `Off` in order for backfaces to be visible. Find this option under [Rendering](/rendering/rendering.mdx).
:::
...
Detail Intensity backface multiplier for detail textures in the [Details](/color-and-normals/details.mdx) section.
...
Custom Multiplier to limit the [Emissions](/special-fx/emission.mdx) shown on the backface.