...
The **Alpha Options** section provides options for modifying how the material treats alpha. The effects of these options are heavily influenced by the [Render Preset](/general/render-preset.mdx) and other render settings.
...
Select which [Global Mask](/modifiers/global-masks.mdx) to use for the Alpha overall.
...
Select which [Global Mask](/modifiers/global-masks.mdx) to only affect the Distance Alpha / Distance Fade onto.
...
This can be used to simulate materials that are more opaque at shallow angles, such as transluscent fabric (used for tights or stockings). It can be thought of as analagous to [Rim Lighting](/shading/rim-lighting.mdx), but for alpha instead of color.
...
Select which [Global Mask](/modifiers/global-masks.mdx) to only affect the Fresnel Alpha effect onto.
...
Select which [Global Mask](/modifiers/global-masks.mdx) to only affect the Angular Alpha effect onto.
