...
- `.poiyomi/Poiyomi {Toon/Pro} World`

The Poiyomi World Shader includes Global Illumination passes. As such, Poiyomi World matches the same features as the normal Poiyomi Shader, with the only difference being that it includes standard Global Illumination support for usage in VRChat Worlds.
...
:::warning[Does Not Support Batching]
Due to how the Shader Optimizer works (see [Locking & Animation](/general/locking.mdx)), Poiyomi Shaders does not support static batching. Please keep this in mind when using the World shader.
:::

## Two Pass

- `.poiyomi/Poiyomi {Toon/Pro} Two Pass`

The Two Pass version renders the inside of the Mesh first, rather than the outside (Cull Front > Cull Back). See [Two Pass](/extended-features/two-pass.mdx) for more info.

Because the Two Pass shader renders everything twice, it can have a slight performance impact.

## Outline Early

- `.poiyomi/Poiyomi {Toon/Pro} Outline Early`
...
- `.poiyomi/Poiyomi {Toon/Pro} Grab Pass`

The Grab Pass Shader is used for specialized effects that require taking a screenshot every frame in order for it to render. See [Grab Pass](/extended-features/grabpass.mdx) for more info.
...
Poiyomi Pro ShatterWave is an advanced special effect that uses the mesh's vertices to create a wavy effect across the mesh in variation. It functions similar to Geometric Dissolve, but does it in a wavy pattern instead. See [ShatterWave](/extended-features/shatterwave.mdx) for more info.

## Tessellated

- `.poiyomi/Poiyomi Pro Tessellated`

The Tessellated shader includes a shading process that subdivides triangles into a more realistic structure for certain realistic effects. See [Tessellated](/extended-features/tessellated.mdx) for more info.
...
Poiyomi Pro Fur is a Geometric Particle shader that uses the 3D mesh to spawn each Particle from, similar to a Particle System in Unity. See [Particle](/extended-features/particle.mdx) for more info.

Just like Geometric Dissolve, it can have a significant performance impact.

## Lil Fur

- `.poiyomi/Poiyomi {Toon/Pro} + Lil Fur`
- `.poiyomi/Poiyomi {Toon/Pro} + Lil Fur Two Pass`

Similar to Poiyomi Fur, but instead uses the same simulation technique on LilToon's Fur Shader. See [Lil Fur](/extended-features/lilfur.mdx) for more details.
