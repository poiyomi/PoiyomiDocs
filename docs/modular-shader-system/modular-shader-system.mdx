...
:::warning[Complex Information Ahead!]
This page contains complex information! Therefore, this is intended to be read by Advanced Shader Developers only.
:::
...
| `[ThryHideInInspector]` | Same as `[HideInInspector]` but allows third-party utilities (such as VRCFury or Modular Avatar) to access it |
...
| `[sRGBWarning(true)]` | Warn if texture is Linear when it shouldn't be |
| `[Gradient]` | Gradient texture picker |
| `[Vector2]` | 2D vector input |
| `[VectorLabel(X, Y)]` | Labeled vector components |
| `[IntRange]` | Only for Int props, makes a clamped range slider determined by `Range(X, Y)` |
| `[Curve]` | Animation curve texture |
| `[MultiSlider]` | Sets the Vector to clamp between a min and max value |
| `[Ramp4(normalized)]` | Use a vector curve ramp |
| `[DoNotAnimate]` | Explicitly block the property from being tagged as Animated |
...
// Nested Subsections
[HideInInspector] ss_start_nestedsubsection ("Nested Subsection--{reference_property_EnableNestedSub}", Float) = 0
// ... properties ...
[HideInInspector] ss_end_nestedsubsection ("", Float) = 0
```
...
These Attriibutes are used soley for helpful and decorative elements, which can help out in UI organization.

```hlsl
[Helpbox(1, 2, 1)] _MessageBox ("This is a UI message.", Int) = 0 // Creates a message box with a message and an Icon

[PoiPrefabSpawner(GUID)] _PrefabSpawner ("Add Prefab to Scene", Float) = 0 // Adds the specific Prefab's GUID directly into the active Scene

[Space(7)] // Creates spacing between the properties above and below this line by 7 pixels

[ThrySeperator(2, 2, 2)] // Draws a horizontal line between the properties above and below this line (useful in Nested Subsections)

// Thry Decal Positioning Tool
[ThryDecalPositioning(_Texture, _UV, _Position, _Rotation, _Scale, _SideOffset)]
// ... properties ...

// Multi-Float Buttons (only used for UV Tile Discard)
...
- [ThryEditor Attributes](/thryeditor/attributes.mdx)
