...
The Light Data section provides options for changing features relating to the data used for lighting and shading the material. Options here significantly influence the functionality of the [Shading](/shading/main.mdx) section.
...
:::note
AO Maps are more effective in Realistic-based lighting methods (see [Shading](/shading/main.mdx)). It may not be visible in Toon-based lighting.
:::
...
## Base Pass (Main Directional & Baked Lights)
...
- `Type`: <PropertyIcon name="dropdown" />**Dropdown**, Options: `Poi Custom`/`Standard`/`UTS2`/`OpenLit (Lil Toon)`
...
Uses the Light Coloring mode used by Lil Toon.
...
Please be aware that using SDF does not support using `Same as Base Pass` in your [Shading](#add-pass-point-spot--other-directional-lights) settings.
...
- `Type`: <PropertyIcon name="dropdown" />**Dropdown**, Options: `Poi Custom`/`Forced Local Direction`/`Forced World Direction`/`UTS2`/`OpenLit (Lil Toon)`/`View Direction`
...
Uses the light direction mode unique to Lil Toon.
...
- `Type`: <PropertyIcon name="floatrange" />**Float**, Range: `-90 - 90`
  - Conditional: Requires [Light Direction Mode](#light-direction-mode) set to `View Direction`
...
- `Type`: <PropertyIcon name="float" />**Vector3**
  - Conditional: Requires [Light Direction Mode](#light-direction-mode) set to `Forced Local Direction` or `Forced World Direction`
...
- `Type`: <PropertyIcon name="color" />**Color**
  - Conditional: Requires [Force Light Color](#force-light-color) to be enabled
...
- `Type`: <PropertyIcon name="floatrange" />**Float**, Range: `0.001 - 4`
  - Conditional: Requires [Light Color Mode](#light-color-mode) set to `UTS2`
...
- `Type`: <PropertyIcon name="floatrange" />**Float**, Range: `0.0 - 10.0`
  - Conditional: Requires [Limit Brightness](#limit-brightness) to be enabled
...
- `Type`: <PropertyIcon name="floatrange" />**Float**, Range: `0.0 - 1.0`
  - Conditional: Requires [Light Color Mode](#light-color-mode) set to `Poi Custom`
...
## Add Pass (Point, Spot, & other Directional Lights)
...
- `Type`: <PropertyIcon name="toggle" />**Toggle**
  - Conditional: Requires [Pixel Lights (Important)](#pixel-lights-important) to be enabled
...
- `Type`: <PropertyIcon name="floatrange" />**Float**, Range: `0.0 - 10.0`
  - Conditional: Requires [Limit Brightness](#limit-brightness-1) to be enabled
...
Remaps the distance attenuation, allowing for finer control of the lighting falloff without having to change it's behavior within Unity.
...
Enables the specular feature in Light Volumes.

### LV Intensity

- `Type`: <PropertyIcon name="floatrange" />**Float**, Range: `0.0 - 1.0`

Configures how intense Light Volumes should affect the material.

### LV Shadows

- `Type`: <PropertyIcon name="dropdown" />**Dropdown**, Options: `Off`/`Soft`/`Medium`/`Sharp`

Configures the type of shadows to simulate when material is exposed to a Light Volume voxel.

### Normal Bias Sampling

- `Type`: <PropertyIcon name="floatrange" />**Float**, Range: `0-1.0 - 1.0`
