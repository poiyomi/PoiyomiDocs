...
:::tip
SSAO rendering is enabled by default. To turn off SSAO in-game, you need to animate this Toggle (checkbox) when creating toggles for SSAO on this Material. Animating the [Section Header](/general/locking.mdx#section-header-checkboxes) will not work!
:::
...
Select which [Global Mask](/modifiers/global-masks.mdx) to apply to SSAO.

### Apply To Global Mask

- `Type`: <PropertyIcon name="dropdown" />**Dropdown**, Options: `Off`/`1R`/`1G`/`1B`/`1A`/`2R`/`2G`/`2B`/`2A`/`3R`/`3G`/`3B`/`3A`/`4R`/`4G`/`4B`/`4A`

Select which [Global Mask](/modifiers/global-masks.mdx) SSAO writes to.
