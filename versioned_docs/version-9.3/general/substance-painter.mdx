...
- `BaseColor`: Place in the [`Main Texture`](/color-and-normals/color-and-normals.mdx#main-texture) slot in [Color & Normals](/color-and-normals/color-and-normals.mdx).
- `NormalMap`: Place in the [`Normal Map`](/color-and-normals/color-and-normals.mdx#normal-map) texture slot in [Color & Normals](/color-and-normals/color-and-normals.mdx). Make sure to mark it as a normal map in the texture import settings.
- `AmbientOcclusion`: Place in the [`AO`](/shading/light-data.mdx#ao-maps) texture slot in [Light Data](/shading/light-data.mdx). Make sure to **uncheck sRGB** in the texture import settings.
...
- `EmissionMap`: Place in the [`Emission Map`](/special-fx/emission.mdx#emission-map) texture slot in [`Emission`](/special-fx/emission.mdx).
- `GlobalMask`: Place in one of the Global Mask texture slots in [`Global Mask`](/modifiers/global-masks.mdx). Make sure to **uncheck sRGB** in the texture import settings. Note that this uses the custom **User channels** in Substance for your Masks. This Template is programmed to use `User10` for the `R` channel, `User11` for the `G` channel, `User12` for the `B` channel, and `User13` for the `A` channel.
