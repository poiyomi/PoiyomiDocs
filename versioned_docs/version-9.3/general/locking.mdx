...
| :--- | :--- |:-----------------------------------------------------------------------------------|
| `_Color`                  | ✅ | [Main Color](/color-and-normals/color-and-normals.mdx#color--alpha)                 |
| `_MainTex_ST`             | ❌ | [Main Texture Tiling/Offset](/color-and-normals/color-and-normals.mdx#main-texture) |
| `_BumpMap_ST`             | ❌ | [Normal Map Tiling/Offset](/color-and-normals/color-and-normals.mdx#normal-map)     |
| `_BumpScale`              | ✅ | [Normal Map Intensity](/color-and-normals/color-and-normals.mdx#normal-map)         |
| `_Cutoff`                 | ✅ | [Alpha Cutoff](/color-and-normals/color-and-normals.mdx#alpha-cutoff)               |
| `_DetailMask_ST`          | ❌ | [Detail Mask Tiling/Offset](/color-and-normals/details.mdx#detail-mask)             |
| `_DetailNormalMap_ST`     | ❌ | [Detail Mask Tiling/Offset](/color-and-normals/details.mdx#detail-normal)           |
| `_DetailNormalMapScale`   | ✅ | [Detail Mask Tiling/Offset](/color-and-normals/details.mdx#normal-intensity)        |
| `_EmissionColor`          | ✅ | [Emission 0 Color](/special-fx/emission.mdx#emission-color)                         |
| `_EmissionMap_ST`         | ❌ | [Emission 0 Map Tiling/Offset](/special-fx/emission.mdx#emission-map)               |
...
- To disable [Color Adjust](/color-and-normals/color-adjust.mdx), you could animate the settings to their default values.
- To disable a [Decal](/color-and-normals/decals.mdx), animate its `Alpha` value to `0`. If the Decal has Emissions, also animate its `Emission Strength` value to `0`.
- To disable [Audio Link](/audio-link/audio-link.mdx), animate the `Anim Toggle` to be *un-checked*.
...
This significantly improves performance, helps mitigate the [64 texture slot crash](/general/textures-64-texture-slot-crash.mdx) sometimes seen when many complex materials with lots of modules enabled are present in a scene.
