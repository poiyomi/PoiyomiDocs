...
**Keep in mind this only works if you set your [Rendering Preset](/general/render-preset.mdx) to Cutout or a Transparent Preset.**
:::
...
This allows you to select which [Global Mask](/modifiers/global-masks.mdx) to affect when it is NOT Dissolved whatsoever.
...
All the sliders listed for each Row will reflect the same locations as described in [UV Tile Discard](/special-fx/uv-tile-discard.mdx). Please refer to the documentation page to see what they are.
...
This property will appear for each Row and Column for UV Tile Dissolve. Below is a table reference matching the same positions as described in [UV Tile Discard](/special-fx/uv-tile-discard.mdx), named to each field shown in UV Tile Dissolve.
