...
:::tip
Animate this slider when creating toggles for LookAt on this Material. Animating the [Section Header](/general/locking.mdx#section-header-checkboxes) will not work!
:::
...
Enables the visibility modifier, which includes features similar to the [Mirror/Camera Visibility](/special-fx/mirror.mdx) feature. This can affect how LookAt appears either Normally, in the Mirror, or in the Camera.
...
Enables [Audio Link](/audio-link/audio-link.mdx) to control LookAt. This will affect ALL `Alpha` properties in LookAt.
